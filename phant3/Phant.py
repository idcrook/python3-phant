...
import requests
import logging
from phant3.encoders import plain_json
import json
...
class Phant(object):
    ...
    def __str__(self):
        return json.dumps(self._jsonKeys, indent=4)
    ...
    def _check_limit_tuple(self, limit_tuple):
        ...
        if len(limit_tuple) != 2:
            raise ValueError(
                "Limit tuple must be of len() == 2.  Got {}".format(
                    len(limit_tuple)))

        if not isinstance(limit_tuple[0], str):
            raise ValueError("Field name must be a string")

        if not isinstance(limit_tuple[1], str):
            raise ValueError("Field limit must be a string")

        if limit_tuple[0] not in self.extended_fields:
            raise ValueError(
                "Field \'{}\' not in the known list of fields: {}".format(
    ...
    def get(self,
        ...
        response = self._get(self.outputUrl(), params=payload_str).json()
        if convert_timestamp:
            ...
            for entry in response:
                ...
                entry['timestamp'] = datetime.datetime.strptime(
                    timestamp, pattern)
        response = [{
            k: self._encoder.deserialize(k, v)
    ...
    def _check_private_key(self, message):
        if not self.privateKey:
            raise ValueError(
                "Must create Phant object with private_key to {}".format(
                    message))

    def _get_stats(self, name=None, force=False):
        if not self._stats or force:
            response = self._get(self.outputUrl('/stats.json'))
